...
tCombo_FontSizePixel=70;


function tComboFrame_OnLoad()
	if TukuiComboText == true then
	...
	BubbleTextString:SetFont("Interface\\AddOns\\Tukui\\media\\font.ttf", tCombo_FontSizePixel, "OUTLINE");
end
...
function tCombo_OnUpdate()
	...
	if ( elapsed < fadeInTime ) then
		...
		this:SetPoint("CENTER", "UIParent", "CENTER", 0, tCombo_FontSizePixel*alpha );
		return;
	end
	...
	if ( elapsed < (fadeInTime + holdTime) ) then
		...
		this:SetPoint("CENTER", "UIParent", "CENTER", 0, tCombo_FontSizePixel );
		return;
	end
